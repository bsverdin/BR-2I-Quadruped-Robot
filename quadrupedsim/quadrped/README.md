...
- A battery/runtime estimator with adjustable voltage, Ah capacity, usable capacity, auxiliary draw, current pessimism factor, internal resistance, and cutoff voltage.
- A red/green motor capability light that checks simulated current and joint torque demand against the motor current limit and the configured gear reduction.
...
Estimate runtime from a simulated gait:

```matlab
config = defaultQuadrupedConfig();
results = simulateQuadrupedGait(config, config.gaits(2), config.motor);
runtime = estimateBatteryRuntime(results, config.battery);
disp(runtime.runtimeMinutes)
disp(runtime.averageCurrentA)
```
...
The displayed hip/knee torque is joint output torque after the gearbox. The motor capability light estimates output torque capacity as:

```matlab
jointTorqueLimit = (MaxCurrent_A - NoLoadCurrent_A) * Kt_Nm_per_A * GearRatio * GearEfficiency
```

The default gear ratio is currently `9:1`.

## Battery Runtime Notes

Click `Battery` after running a gait to estimate battery runtime. The estimator sums the eight simulated motor current traces, applies `currentSafetyFactor`, then adds auxiliary draw:

```matlab
inputCurrentA = currentSafetyFactor * sum(motorCurrentA) + auxiliaryDrawA + auxiliaryDrawW / batteryVoltage
usedAh = cumtrapz(time, inputCurrentA) / 3600
```

The runtime estimate uses usable capacity:

```matlab
usableAh = capacityAh * usableCapacityPct / 100
runtimeMinutes = usableAh / averageInputCurrentA * 60
```

This is a first-pass estimate from torque-derived motor/controller current. Calibrate it later with real battery current and voltage logs before final pack sizing.
