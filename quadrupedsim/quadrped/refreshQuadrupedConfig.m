...
% In refreshQuadrupedConfig.m (or wherever you set defaults)
if ~isfield(config, 'hipMountRoll') || isempty(config.hipMountRoll)
    config.hipMountRoll =[0, 0,  0, 0];
end
if ~isfield(config, 'hipMountYaw') || isempty(config.hipMountYaw)
    config.hipMountYaw =[0, 0,  0, 0];
end
